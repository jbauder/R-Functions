...
# Initialize windows for carts
	...
	dev.set(2)
	par(mar=rep(.1,4))			# Set reasonable margins for tree
...
# my laptop, sdi interface (used by turtledisp workspace)
		...
		windows(5.9,4.7,xpos=1098,ypos=515)
		windows(5.9,4.7,xpos=1098)
		}
...
"cart" <-
function(formula, data, ..., control=usual, pick = TRUE, prune = 0, textsize = 0.75, smooth = 1, margin=.06, weight = NULL)
{
...
#Works properly with weights= (very convoluted), 25 Aug 2008
	...
	if(smooth > 1) {
		q <- z$cptable
		for(i in 2:smooth) {
			q <- q + rpart(formula, data, weightval, control=control, ...)$cptable
		}
		z$cptable <- q/smooth
	}
	rm('weightval', inherits=TRUE)
	if(prune != 0)
...
# 5 May 2009: Fixed error from '[\.]', thanks to R update
	...
	if(l == 0) {											# if top level, prepare data
		q <- data.frame(as.numeric(row.names(a$frame)), gsub("[.]","_",I(as.character(a$frame$var))),
...
# Requires global dataset (named in x$data)
...
# 16 Nov 2006: update to use data.frame instead of list thx to R 2.4.0 update.  Grr.
...
if(chatter) cat(paste('node ',i,', primary ',p[1],', calc. i = ',round(p2,8),'\n', sep=''))
			...
			for(j in s) {										#	For each surrogate at this node (j points to x$splits),
				...
				t <- data.frame(g[j,'variable'],z); names(t) <- names(m)
				m <- rbind(m,t)
